...
def sample_or_pad_matrix(matrix, shape=(256, 256)):
    ...
    if m <= 256:
        ...
        nums = np.random.choice(m, shape[0])
        print(nums)
        print(nums.shape)
    return new_matrix
...
def main(seqfile, threads=1, output_dir='./'):
    ...
    print('Padding or sampling ccm to fixed size...')
    mat_file = prefix + '.mat'
    ccm_mat = np.loadtxt(ccm_file)
    print(ccm_mat.shape)
    fix_mat = sample_or_pad_matrix(ccm_mat, (256, 256))
    print(fix_mat.shape)
    print(fix_mat)
